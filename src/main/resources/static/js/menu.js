...
document.querySelectorAll(".dropdown-menu li a").forEach(link => {
    ...
    function filterItems(categoryID) {
        ...
        document.querySelectorAll('.sidebar li').forEach(btn => btn.classList.remove('active'));
        const activeBtn = document.querySelector(`.sidebar li[data-cat="${categoryID}"]`);
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.scrollToCategory = function(categoryId) {
        const targetSectionId = categoryMapping[categoryId] || 'all';
        const targetSection = document.getElementById(targetSectionId);
        const correspondingButton = document.querySelector(`[data-cat="${categoryId}"]`);
